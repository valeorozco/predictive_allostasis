...
import math
import csv
#output from the ff controllers
out_list = []
out_thirst_list = []
temperature = True
thirst = True
window_size = 3 #Number of episodes gathering error to train the feedforward
...
class Allostasis():
    ...
    def __init__(self):
        ...
        self.target_smooth_temp = 0
        self.target_smooth_thirst= 0
...
########################### ATTRACTOR DYNAMICS ##########################
    def attractor_dynamics(self):
        """
        Calls main functions from the attractor class in which conflict resolution from both needs takes place, generates two outputs.
        """
        self.Itemp_attractor *= 10
        self.Ithi_attractor *= 10
        input_thirst = self.output_thirst * self.Ithi_attractor
        input_temp = self.output * self.Itemp_attractor
        self.total_force_temperature, self.total_force_thirst = self.attractor.advance(input_temp, input_thirst)
...
########################### SAVING DATA ###########################
    ...
    def save_data(self, current_simulation):
        global aVtemperature_list, aVthirst_list, Drive_temperature_list, Drive_thirst_list, TFtemperature_list, TFthirst_list, meanGrad_Temp, _list, Iatt_temperature_list, out_list
        csv_namefile = 'THESIS/Results/FF/testFF' + str(current_simulation+1) + '.csv'
        print(csv_namefile)
        
        with open(csv_namefile, mode='w') as csv_file:
            csv_writer = csv.DictWriter(csv_file, fieldnames=['Xposition', 'Yposition', 'aVtemperature', 'aVthirst', 'Error_to_attractor', 'DriveThirst', 'TFtemperature', 'TFthirst', 'Thirst_error','Temperature_error', 'Grad_Temp','Output','Output_thirst'])
            ...
            for i in range(window_size, episodes):
                csv_writer.writerow({'Xposition': self.robot_x[i], 'Yposition': self.robot_y[i], 'aVtemperature': aVtemperature_list[i], 'aVthirst': aVthirst_list[i],
                  ...
                  'Thirst_error':Iatt_thirst_list[i],'Temperature_error':Iatt_temperature_list[i],'Grad_Temp': meanGrad_Temp[i], 'Output': out_list[i-window_size], 'Output_thirst':out_thirst_list[i-window_size]}) 

    def clean(self):
        global aVtemperature_list, aVthirst_list, Drive_temperature_list, Drive_thirst_list, TFtemperature_list, TFthirst_list, Iatt_temperature_list, Iatt_thirst_list, out_list, out_thirst_list
        self.robot_x = [random.randint(10,190)]
        self.robot_y = [random.randint(10,190)]
        self.gradient_position_float = 0
...
####################### FEEDFORWARD ###################################
    def feedforward(self):
        """ 
        Calls the main functions of the feedforward controller and generates two outputs, one per controller
        """
        input_temp = np.array([[[self.temperature_intensity.mean()]]])
        error_temp =  np.array([self.Itemp_attractor])

        self.output = ff_module.activate(input_temp, error_temp, update=True)
        self.output = self.output[0]
        out_list.append(self.output)
   
        input_thirst = np.array([[[self.temperature_intensity.mean()]]])
        error_thirst = np.array([self.Ithi_attractor])

        self.output_thirst = ff_thirst.activate(input_thirst, error_thirst, update=True)
        self.output_thirst = self.output_thirst[0]
        out_thirst_list.append(self.output_thirst)    
...
ff_module = Cerebellum(dt=1e-3, nInputs=1, nOutputs=1, nPCpop=10, nIndvBasis=100, nSharedBasis=200, beta_MF=1e-3, 
                       beta_PF=1e-9, range_delays=[0.05,0.5], range_TC=[0.05, 2.], range_scaling=[1, 100], range_W=[0., 1.])
ff_thirst = Cerebellum(dt=1e-3, nInputs=1, nOutputs=1, nPCpop=10, nIndvBasis=100, nSharedBasis=200, beta_MF=1e-3, 
                       beta_PF=1e-9, range_delays=[0.05,0.5], range_TC=[0.05, 2.], range_scaling=[1, 100], range_W=[0., 1.])                       
