...
temperature = True
thirst = True
target_smooth_temperature_list = []
target_smooth_thirst_list = []
...
########################### ATTRACTOR DYNAMICS ##########################
    def attractor_dynamics(self):#output
...
########################### SAVING DATA ###########################
    ...
    def save_data(self, current_simulation):
        global aVtemperature_list, aVthirst_list, Drive_temperature_list, Drive_thirst_list, TFtemperature_list, TFthirst_list, meanGrad_Temp, target_smooth_temperature_list, Iatt_temperature_list, out_list, log_err
        csv_namefile = '/home/robotics20/Documents/Allostasis_TFM/Predictive_Allostasis-master/THESIS/Results/NoFF/noff_test' + str(current_simulation+1) + '.csv'
        print(csv_namefile)
        
        with open(csv_namefile, mode='w') as csv_file:
            csv_writer = csv.DictWriter(csv_file, fieldnames=['Xposition', 'Yposition', 'aVtemperature', 'aVthirst', 'DriveTemp', 'DriveThirst', 'TFtemperature', 'TFthirst', 'Temperature_error', 'Thirst_error','Grad_Temp'])
            ...
            for i in range(window_size, episodes):
                csv_writer.writerow({'Xposition': self.robot_x[i], 'Yposition': self.robot_y[i], 'aVtemperature': aVtemperature_list[i], 'aVthirst': aVthirst_list[i],
                    ...
                    'Temperature_error':Iatt_temperature_list[i],'Thirst_error':Iatt_thirst_list[i],'Grad_Temp': meanGrad_Temp[i]}) 

    def clean(self):
        global aVtemperature_list, aVthirst_list, Drive_temperature_list, Drive_thirst_list, TFtemperature_list, TFthirst_list, Iatt_temperature_list, Iatt_thirst_list, target_smooth_temperature_list, out_list, log_err
        self.robot_x = [random.randint(10,190)]
        self.robot_y = [random.randint(10,190)]
        self.gradient_position_float = 0
...
####################### FEEDFORWARD ###################################
    ...
    def feedforward(self, i, meanGrad_Temp, target_smooth_temperature_list):
        """ 
        Calls the main functions of the feedforward controller and generates an output 
        
        Arguments:
        1.i(int) - index of the run loop
        2.meanGrad_Temp (float[]list) - size of the grid
        3. target_smooth_list(float[]list) - 
            """
        
        cue, target = ff_module.create_signals(meanGrad_Temp[window_size:], target_smooth_temperature_list)
        self.output = ff_module.learn(cue, target, i-window_size)[0]    #error thirst
        out_list.append(self.output)

###################### MEAN SQUARED ERROR ###########################
    def MSE(self):
        """
        Fills a list with the values of the error list and squares them
            """
        log_err.append(np.mean(np.power(ff_module.err[1:],2)))
        #print(log_err)

    ###################### TARGET SMOOTH ################################
    def target_smooth_temperature(self,i):
        """ 
    Generates an averaged signal with the Iatt_temperature_list values.
    
    Arguments:
    1.i(int) - index of the run loop
        """
        if (i+1) >= window_size: #creates target smooth with window_size
            target_smooth_temp = np.mean(Iatt_temperature_list[-window_size:])
            target_smooth_temperature_list.append(target_smooth_temp)
            
        """ target_smooth_thirst = np.mean(Iatt_thirst_list[-window_size:])
        target_smooth_thirst_list.append(target_smooth_thirst)     
...
########################### RUN SIMULATION ###########################
    def run(self, current_simulation):
        ...
        for i in tqdm(range(episodes)):
            ...
            self.hsign()
            self.homeostasis()
            #self.target_smooth_temperature(i)
            #if i>=window_size:
            #    self.feedforward(i,meanGrad_Temp,target_smooth_temperature_list)
            #    self.MSE()
...
allo = Allostasis()
ff_module = Cerebellum(dt=1e-2, nInputs=1, nOutputs=1, nPCpop=10, nIndvBasis=100, nSharedBasis=200, beta_MF=1e-3, 
                       beta_PF=1e-6, range_delays=[0.05, 0.5], range_TC=[0.05, 2.], range_scaling=[1, 100], range_W=[0., 1.])
